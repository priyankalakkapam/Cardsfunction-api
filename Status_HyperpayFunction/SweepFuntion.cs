...
namespace Status_HyperpayFunction
{
    public class SweepFuntion
    {
        ...
        private static List<withdrawalTransactions> withdrawalPendingTransactions()
        {
            ...
            string query = "select CustomerId,Id,State,TxRef,IsCompleted,RetryCount from Finance.[Transaction] where TxType = 'Withdraw' and TxSubType = 'crypto' and State = 'Pending' and TxSource = 'Exchange' AND (RetryCount < 3 OR RetryCount IS NULL)  AND (IsCompleted = 0 OR IsCompleted IS NULL) and ProductId is null order by TxDate desc";
